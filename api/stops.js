module.exports = async function handler(req, res) {
  ...
  if (!MTA_KEY) {
    return res.status(200).json({ stops: [], error: 'MTA_API_KEY not set in Vercel env' });
  }
  if (!minLat) {
    return res.status(400).json({ error: 'bounds required: minLat, maxLat, minLng, maxLng' });
  }
  ...
  try {
    ...
    if (!r.ok) throw new Error(`MTA API returned ${r.status}`);
